...
class DeployFunction extends AbstractHandler {
    ...
    async updateConfiguration(ns, oldFunc, funcObject) {
        const configArgs = {};
        if (!oldFunc) {
            if (funcObject.Properties.Description)
                configArgs.Description = funcObject.Properties.Description;
            if (funcObject.Properties.Runtime)
                configArgs.Runtime = funcObject.Properties.Runtime;
            if (funcObject.Properties.Role)
                configArgs.Role = funcObject.Properties.Role;
            if (funcObject.Properties.ClsLogsetId)
                configArgs.ClsLogsetId = funcObject.Properties.ClsLogsetId;
            if (funcObject.Properties.ClsTopicId)
                configArgs.ClsTopicId = funcObject.Properties.ClsTopicId;
            if (funcObject.Properties.VpcConfig) 
                configArgs.VpcConfig = funcObject.Properties.VpcConfig;
        } else {
            if (funcObject.Properties.Description && oldFunc.Description != funcObject.Properties.Description)
                configArgs.Description = funcObject.Properties.Description;
            if (funcObject.Properties.Runtime && oldFunc.Runtime != funcObject.Properties.Runtime)
                configArgs.Runtime = funcObject.Properties.Runtime;
            if (funcObject.Properties.Role && oldFunc.Role != funcObject.Properties.Role)
                configArgs.Role = funcObject.Properties.Role;
            if (funcObject.Properties.ClsLogsetId && oldFunc.ClsLogsetId != funcObject.Properties.ClsLogsetId)
                configArgs.ClsLogsetId = funcObject.Properties.ClsLogsetId;
            if (funcObject.Properties.ClsTopicId && oldFunc.ClsTopicId != funcObject.Properties.ClsTopicId)
                configArgs.ClsTopicId = funcObject.Properties.ClsTopicId;
            if (!_.isEmpty(funcObject.Properties.VpcConfig) && 
                (oldFunc.VpcConfig.VpcId != funcObject.Properties.VpcConfig.VpcId || 
                oldFunc.VpcConfig.SubnetId != funcObject.Properties.VpcConfig.SubnetId)) 
                configArgs.VpcConfig = funcObject.Properties.VpcConfig;
        }

        const envs = [];
        const oldEnvs = [];
        if (oldFunc && oldFunc.Environment && oldFunc.Environment.Variables && 
            funcObject.Properties.Environment && _.isObject(funcObject.Properties.Environment.Variables)) {
            for (let key in funcObject.Properties.Environment.Variables) {
                const value = funcObject.Properties.Environment.Variables[key];
                const oldItem = _.find(oldFunc.Environment.Variables, (data) => {
                    if (data.Key == key && data.Value == value)
                        return data;
                });
                if (!_.isEmpty(oldItem)) {
                    oldEnvs.push(oldItem);
                    continue;
                }
        ...
        if (!_.isEmpty(envs)) {
            configArgs.Environment = {
                Variables: [...envs, ...oldEnvs]
            };
        } else
        if (oldFunc && oldFunc.Environment && _.size(oldEnvs) != _.size(oldFunc.Environment.Variables)) {
            configArgs.Environment = {
                Variables: oldEnvs
            };
        } 
        ...
        // delete all evn
        if (oldFunc && oldFunc.Environment && _.size(oldFunc.Environment.Variables) > 0 &&
                configArgs.Environment && _.size(configArgs.Environment.Variables) == 0) {
            configArgs.Environment = {
                Variables: [{
                    Key: '',
                    Value: ''
