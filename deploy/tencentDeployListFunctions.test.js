...
describe('TencentDeployListFunctions', () => {
    ...
    describe('#constructor()', () => {
        ...
        describe('hooks', () => {
            ...
            beforeEach(() => {
                validateStub = sinon.stub(tencentDeployListFunctions, 'validate')
                    .returns(Promise.resolve());
                setDefaultsStub = sinon.stub(tencentDeployListFunctions, 'setDefaults')
                    .returns(Promise.resolve());
                tencentDeployListFunctionsStub = sinon.stub(tencentDeployListFunctions, 'functionList')
                    .returns(Promise.resolve());
            });
            ...
            it('should run "before:deploy:list:functions:log" promise chain', () => tencentDeployListFunctions
                .hooks['before:deploy:list:functions:log']().then(() => {
                    validateStub.calledOnce.should.equal(true);
                    setDefaultsStub.calledAfter(validateStub).should.equal(true);
                }));

            it('should run "deploy:list:functions:log" promise chain', () => tencentDeployListFunctions
                .hooks['deploy:list:functions:log']().then(() => {
