...
class TencentInfo {
    constructor(serverless, options) {
        ...
        this.hooks = {
            'before:metrics:metrics': () => BbPromise.bind(this)
                .then(this.validate)
    ...
    async metrics() {
        try {
            ...
            if (functionListData.length > 0) {
                for (let eveFunctionIndex = 0; eveFunctionIndex < functionListData.length; eveFunctionIndex++) {
                    ...
                    const functionDuration = await Handler.getMonitor(
                        ...
                        startTime,
                        endTime);
                    const duration = this.getSum(functionDuration.DataPoints[0].Values) / this.getSum(functionInvocation.DataPoints[0].Values)
                ...
                let functionStr = "";
                for (let key in functionInformation) {
                    ...
                    functionStr = functionStr + "  " + key + ": " + "\n    Invocations: " + functionInformation[key]['Invocation'] + "\n" +
                        "    Outflows: " + functionInformation[key]['OutFlow'] + "\n" +
                        "    Errors: " + functionInformation[key]['Error'] + "\n" +
                        "    Duration(avg.): " + functionInformation[key]['Duration'] + " ms\n\n"
                }
                output = output + "\nService:\n  Invocations: " + serviceInvocation + "\n" +
                    "  Outflows: " + serviceOutFlow + "\n" +
                    "  Errors: " + serviceError + "\n" +
                    "  Duration(avg.): " + serviceDuration + " ms\n\nFunctions: \n" + functionStr
            } else {
